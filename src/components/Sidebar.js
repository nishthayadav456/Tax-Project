...
const Sidebar = ({
  ...
  useEffect(() => {
    ...
    async function fetchCompanyDetails() {
      ...
      if (response) {
        ...
        const logourl =
          response.data.data[response.data.data.length - 1].companyLogo;
        const Nameurl =
          response.data.data[response.data.data.length - 1].companyName;
  ...
  return (
    <>
      <div
        className={`bg-orange-400 text-white h-full w-1/4 sm:w-1/5 md:w-1/7 lg:w-1/7 xl:w-1/8 ${
          isSidebarOpen ? "block" : "hidden"
        }`}
      >
        <div className="lg:block">
          <div
            className="flex items-center p-5 shadow border-black"
          ...
          <div>
            ...
            <div>
              ...
              <div
                className={`overflow-hidden transition-all duration-300 px-6 bg-orange-900 rounded ${
                  salesExpanded ? "max-h-screen" : "max-h-0"
                }`}
              >
                <div className=" py-2">
                  <Link to="/sales-invoice" className="block py-2 flex px-2">
                    Sales Invoices
                  </Link>
                </div>
                <div className=" py-2">
                  <Link to="/estimate" className="block py-2 flex px-2">
                    Estimate
                  </Link>
                </div>
                <div className=" py-2">
                  <Link to="/payment-in" className="block py-2 flex px-2">
                    Payment In
                  </Link>
                </div>
                <div className=" py-2">
                  <Link to="/sale-order" className="block py-2 flex px-2">
                    Sale Order
                  </Link>
                </div>
              </div>
            ...
            <div>
              ...
              <div
                className={`overflow-hidden transition-all duration-300 px-6 bg-orange-900 rounded ${
                  purchaseExpanded ? "max-h-screen" : "max-h-0"
                }`}
              >
                <div className=" py-2">
                  <Link to="/purchase-bill" className="block py-2 flex px-2">
                    Purchase Bills
                  </Link>
                </div>
                <div className=" py-2">
                  <Link to="/payment-out" className="block py-2 flex px-2">
                    Payment Out
                  </Link>
                </div>
                <div className=" py-2">
                  <Link to="/purchase-order" className="block py-2 flex px-2">
                    Purchase Order
                  </Link>
                </div>
                <div className=" py-2">
                  <Link to="/purchase-return" className="block py-2 flex px-2">
                    Purchase Return
                  </Link>
                </div>
              </div>
            ...
            <div>
              ...
              >
                <div className="flex">
                  ...
                  <p className="px-2">Cash, bank & Assets</p>
                </div>
                {bankExpanded ? (
                  <svg
                    class="w-6 h-6 text-gray-800 dark:text-white"
              ...
              <div
                className={`overflow-hidden transition-all duration-300 px-6 bg-orange-900 rounded ${
                  bankExpanded ? "max-h-screen" : "max-h-0"
                }`}
              >
                <div className=" py-2">
                  <Link to="/add-bank-account" className="block py-2 flex px-2">
                    Add Bank Account
                  </Link>
                </div>
                <div className=" py-2">
                  <Link to="/cash-in-hand" className="block py-2 flex px-2">
                    Cash In Hand
                  </Link>
                </div>
                <div className=" py-2">
                  <Link to="/cheques" className="block py-2 flex px-2">
                    Cheques
                  </Link>
                </div>
                <div className=" py-2">
